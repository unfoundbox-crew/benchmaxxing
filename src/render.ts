...
function finiteScore(value: number, benchmark: Benchmark): number {
...
function ensureValid(options: BenchmaxxOptions): void {
  if (!options.models.length) throw new Error("At least one model is required.");
  if (!options.benchmarks.length) throw new Error("At least one benchmark is required.");
  ...
  for (const benchmark of options.benchmarks) {
    for (const id of Object.keys(benchmark.scores)) {
      if (!ids.has(id)) throw new Error(`Benchmark ${benchmark.name} references unknown model ${id}.`);
    }
    for (const model of options.models) {
      const score = benchmark.scores[model.id];
      if (score === undefined) throw new Error(`Benchmark ${benchmark.name} is missing score for ${model.id}.`);
...
function panelSvg(
  ...
  parts.push(`<rect x="0" y="0" width="${width}" height="${height}" rx="16" fill="${options.panelColor}"/>`);
  parts.push(`<text x="${padX}" y="${titleY}" font-size="17" font-weight="700" fill="${options.foreground}">${escapeXml(benchmark.name)}</text>`);
  if (benchmark.note) {
    parts.push(`<text x="${width - padX}" y="${titleY}" text-anchor="end" font-size="10" fill="${options.foreground}" opacity="0.45">${escapeXml(benchmark.note)}</text>`);
  }
  for (const tick of [0, max / 2, max]) {
    const ty = chartBottom - chartHeight * (tick / max);
    parts.push(`<line x1="${padX + axisW}" y1="${ty.toFixed(2)}" x2="${width - padX}" y2="${ty.toFixed(2)}" stroke="${options.foreground}" opacity="0.12"/>`);
    parts.push(`<text x="${padX + axisW - 5}" y="${(ty + 3).toFixed(2)}" text-anchor="end" font-size="9.5" fill="${options.foreground}" opacity="0.5">${escapeXml(fmt(tick))}</text>`);
  }
  ...
  options.models.forEach((model, i) => {
    ...
    if (barY - 7 >= chartTop + 30) {
      parts.push(`<text x="${labelX}" y="${Math.max(chartTop + 15, barY - 22).toFixed(2)}" text-anchor="middle" font-size="13" font-weight="${nameWeight}" fill="${isHighlight ? options.highlightColor : options.foreground}">${escapeXml(label)}</text>`);
      parts.push(`<text x="${labelX}" y="${Math.max(chartTop + 30, barY - 7).toFixed(2)}" text-anchor="middle" font-size="11" font-weight="${isHighlight ? 800 : 600}" fill="${isHighlight ? options.highlightColor : options.foreground}" opacity="${isHighlight ? 1 : 0.62}">${escapeXml(fmt(score) + (benchmark.suffix ?? ""))}</text>`);
    } else {
...
export function benchmaxx(options: BenchmaxxOptions): string {
  ensureValid(options);
  ...
  const panelColor = options.panelColor ?? defaults.panelColor;
  ...
  if (options.title) {
    parts.push(`<text x="${outerPad}" y="48" font-size="30" font-weight="820" fill="${foreground}">${escapeXml(options.title)}</text>`);
  }
  if (options.subtitle) {
    parts.push(`<text x="${outerPad}" y="78" font-size="15" fill="${foreground}" opacity="0.58">${escapeXml(options.subtitle)}</text>`);
  }
  ...
  for (const model of options.models) {
    ...
    parts.push(`<text x="${cursorX}" y="${legendY}" font-size="12" font-weight="${isHighlight ? 800 : 600}" fill="${isHighlight ? highlightColor : foreground}" opacity="${isHighlight ? 1 : 0.62}">${escapeXml(label)}</text>`);
