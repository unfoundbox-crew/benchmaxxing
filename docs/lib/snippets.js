...
function checkBms(bms) {
    if (!Array.isArray(bms) || bms.length === 0)
        fail("need at least one benchmark");
    for (const b of bms) {
        if (typeof b?.name !== "string" || b.name === "")
            fail("every benchmark needs a name");
        if (typeof b?.scores !== "object" || b.scores === null)
            fail(`benchmark "${b?.name}" needs a scores object`);
        for (const v of Object.values(b.scores)) {
            if (typeof v !== "number" || !Number.isFinite(v))
                fail(`benchmark "${b.name}" has a non-numeric score`);
        }
    }
}
function modelsOf(bms) {
...
export function renderMarkdownTable(bms) {
    checkBms(bms);
    const models = modelsOf(bms);
    if (models.length === 0)
        fail("no model scores to render");
...
export function renderHfCard(bms, provenance) {
    checkBms(bms);
    if (typeof provenance !== "string" || provenance.trim() === "")
        fail("renderHfCard needs a provenance string");
    return ("---\n" +
        "license: mit\n" +
...
export function renderPost(bms, links = {}) {
    checkBms(bms);
    const models = modelsOf(bms);
    if (models.length === 0)
        fail("no model scores to render");
    ...
    const tops = bms.map((b) => {
        ...
        for (const m of models) {
            if ((b.scores[m] ?? Number.NEGATIVE_INFINITY) > (b.scores[best] ?? Number.NEGATIVE_INFINITY))
                best = m;
        }
