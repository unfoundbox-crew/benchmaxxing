...
import { finiteScore, fmt, highlightScale, resolveMax, resolveTheme } from "./shared.js";
...
function ensureValidOg(options) {
    if (!options.models.length)
        fail("at least one model is required.");
    if (!options.benchmarks.length)
        fail("at least one benchmark is required.");
    if (options.benchmarks.length > OG_MAX_BENCHMARKS) {
        fail(`og layout supports at most ${OG_MAX_BENCHMARKS} benchmarks (got ${options.benchmarks.length}); use --tasks to select a subset.`);
    }
    ...
    for (const benchmark of options.benchmarks) {
        for (const id of Object.keys(benchmark.scores)) {
            if (!ids.has(id))
                fail(`benchmark ${benchmark.name} references unknown model ${id}.`);
        }
        for (const model of options.models) {
            const score = benchmark.scores[model.id];
            if (score === undefined)
                fail(`benchmark ${benchmark.name} is missing score for ${model.id}.`);
            finiteScore(score, benchmark);
        }
    }
}
function rowGroup(benchmark, options, y, height, labelX, barsX, barsW, width) {
    const titleY = 18;
    const barsTop = 30;
...
export function renderOg(options) {
